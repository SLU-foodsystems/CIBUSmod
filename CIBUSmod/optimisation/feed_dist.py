...
class FeedDistributor(GeoDistributor):
    ...
    def make_C10(self):
        ...
        """
        # Fetch the amount of generated byproducts as calculated in the DemandsAndConversions-module
        ...
        # Demand side: feed allocations consuming by-products (negative sign)
        A10_fds_demand = self.make_A1_3(
            row_idx = D_byprod.index,
            prod_type = "by_prod"
        )

        # Generation side: feed allocations generating by-products (positive sign)
        A10_fds_gen = self.make_A1_3_gen(row_idx=D_byprod.index)

        # Net feed contribution = generation - demand  (demand matrix already has -ve sign)
        A10_fds_net = IndexedMatrix(
            A10_fds_demand.M + A10_fds_gen.M,
            D_byprod.index,
            {"fds": self.x_idx["fds"]},
        )

        # Zero matrices for animal and crop columns
        Z_ani = scipy.sparse.csc_array((A10_fds_net.M.shape[0], len(self.x_idx["ani"])))
        Z_crp = scipy.sparse.csc_array((A10_fds_net.M.shape[0], len(self.x_idx["crp"])))

        M = IndexedMatrix(
            scipy.sparse.hstack([Z_ani, Z_crp, A10_fds_net.M], format="csc"),
            D_byprod.index,
            {
                "ani": self.x_idx["ani"],
                "crp": self.x_idx["crp"],
    ...
    def make_C15(self):
        ...
        A15_3_gen = self.make_A15_3_gen(row_idx)

        # Construct final matrix: supply (A15_1, A15_2, A15_3_gen) - demand (A15_3) >= 0
        A15 = IndexedMatrix(
            scipy.sparse.hstack([A15_1.M, A15_2.M, A15_3.M + A15_3_gen.M]),
            row_idx=row_idx,
            col_idx={
                "ani": self.x_idx["ani"],
                "crp": self.x_idx["crp"],
    ...
    def make_A15_3_gen(self, row_idx):
        """
