...
class Regions(object):
        ...
        for k, v in settings.items():
            if k in self.settings:
                if type(v) is type(self.settings[k]):
                    ...
                    raise TypeError(
                        f'Expected {type(self.settings[k])} for setting "{k}"'
                    )
            else:
    ...
    def get_x0(self):
        def get_data(parameter: str, columns: list[str], series_name: str):
            """
            Auxiliary function for creating a series with a MultiIndex, getting
            data from the ParameterRetriever.
            """
            self.par.clear()
            param_values = self.par.get_unique(
                columns, qry=f'parameter == "{parameter}"'
            )
            param_values = map(tuple, param_values.values)
            all_regions = self.par.get_unique(
                "region", qry=f'parameter == "{parameter}"'
            )
            idx = pd.MultiIndex.from_tuples(
                [tup + (re,) for tup in param_values for re in all_regions],
                names=[*columns, "region"],
            )

            return pd.Series(
                self.par.get(parameter, **idx.to_frame().to_dict("list")),
                index=idx,
                name=series_name,
            ).sort_index()

        # Get x0_crops
        x0_crp = get_data("x0_crops", ["crop", "prod_system"], "area")
        x0_ani = get_data("x0_animals", ["species", "breed", "prod_system"], "number")
    ...
    def get_climate(self):
        ...
        climate = pd.DataFrame(np.array([p("GDD5")]).T, index=index, columns=["GDD5"])
    ...
    def classify_soil_texture(self):
        ...
        soil["texture class"] = soil.apply(
            _get_soil_class, txt_classes=txt_classes, axis=1
        )
    ...
    def classify_soil_PK(self, element):
        ...
        for k, v in classes.items():
            soil[f"{element}_class"] = soil[f"{element}_class"].where(
                (soil[f"{element}_AL"] > v) | ~soil[f"{element}_class"].isna(), k
            )
...
def _get_soil_class(x, txt_classes):
    for txt_class in txt_classes:
        if _point_in_polygon(
            [x["sand"] / 100, x["clay"] / 100], txt_classes[txt_class]
        ):
            return txt_class
    return np.nan
