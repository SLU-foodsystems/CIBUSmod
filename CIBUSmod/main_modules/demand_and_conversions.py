...
class DemandAndConversions(object):
    ...
    def calculate_product_demand_and_by_products(self):
        ...
        demand = pd.concat(
            (
                self.data_attr.get("food_demand_to_processing")["domestic"].rename(
                    "food"
                ),
        ...
        animal_prod_demand, animal_by_products = _get_demand(
            self, demand, ["species", "animal_prod"]
        )
        crop_resid_demand, crop_resid_by_products = _get_demand(
            self, demand, "crop_resid"
        )
        by_prod_demand, by_prod_by_products = _get_demand(self, demand, "by_prod")

        # Handle "cream balance" by inducing skim milk exports
        try:
            animal_prod_demand, animal_by_products, by_prod_demand = _fix_cream_balance(
                self, animal_prod_demand, animal_by_products, by_prod_demand
            )
        except ValueError:
            # Ignore if fix cream balance fails
            pass

        # Combine dataframes of generated by-products
        by_products = (
            crop_by_products.add(animal_by_products, fill_value=0)
...
def _fix_cream_balance(self, animal_prod_demand, animal_by_products, by_prod_demand, induce_export_of = 'Milk powder'):
    ...
    if len(food_group) == 0:
        raise ValueError(f"No food_groups for food group '{induce_export_of}' found.")
