...
class DemandAndConversions(object):
    ...
    def get_non_food_demand(self):

        # Get food items included in the diet and included production systems
        fs = self.par.get_unique(['food','food_group'], 'parameter == "non_food_demand"')
        pss = ['conventional'] # All non-food demand assumed conventional
    ...
    def calculate_crop_product_demand(self):
        ...
        crop_prod_demand = pd.DataFrame(
            columns = pd.MultiIndex.from_tuples(
                [(d,cp) for d in ['food','non-food'] for cp in cps ],
        ...
        crop_by_products = pd.DataFrame(
            columns = pd.MultiIndex.from_tuples(
                [(d,cp,bp) for d in ['food','non-food'] for cp,bp in bps.values],
    ...
    def calculate_animal_product_demand(self):
        ...
        animal_prod_demand = pd.DataFrame(
            columns = pd.MultiIndex.from_tuples(
                [(d,sp,ap) for d in ['food','non-food'] for sp,ap in aps.values],
        ...
        animal_by_products = pd.DataFrame(
            columns = pd.MultiIndex.from_tuples(
                [(d,sp,ap,bp) for d in ['food','non-food'] for sp,ap,bp in bps.values],
