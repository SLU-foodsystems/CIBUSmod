...
class FeedDistributor(GeoDistributor):
    ...
    def make_C2(self):
        ...
        factors = self._get_feed_to_prod_factors(index=True, drop_region=False)
        factors_with_reg_share = factors[factors["share_regional"] > 0]

        row_idx = (
            factors_with_reg_share
            .reset_index()
            .set_index(['prod_system','crop_prod','region'])
            .index.unique()
        )
    ...
    def make_C8(
        ...
        # Call generic make function (GeoDistributor)
    ...
    def make_C10(self):
        """
        Ensure the national production of by-products meets demand from feeds and
        other uses. By-product supply is expressed as a function of x_ani and x_crp
        (analogous to C15 at regional level) rather than a fixed pre-computed scalar.

        Constraint form: [A10_ani | A10_crp | A10_fds_net] @ x >= b10
        where b10 is the fixed food/non-food/export demand for by-products.
        """
        # Fixed demand: food/non-food/export uses of by-products, aggregated to
        # (prod_system, by_prod) level
        by_prod_demand = self.demand.data_attr.get("by_prod_demand")
        if by_prod_demand.shape[0] > 0:
            b10_fixed = (
                by_prod_demand
                .sum(axis=1)
                .groupby(level=["prod_system", "by_prod"])
                .sum()
            )
        else:
            b10_fixed = pd.Series(
        ...
        # Collect all by-products appearing across feeds, fixed demand, and
        # crop/animal supply pathways
        bp_feeds = set(
            self.feed_mgmt.par.get_unique("by_prod", qry="parameter == 'feed_to_prod'")
        )
        bp_feed_gen = set(
            self.feed_mgmt.par.get_unique("by_prod", qry="parameter == 'feed_to_by_prod'")
        )
        bp_demand = set(b10_fixed.index.unique("by_prod"))
        by_per_cp = self.demand.data_attr.get("by_prod_per_crop_prod")
        bp_crops = set(by_per_cp.index.unique("by_prod")) if len(by_per_cp) > 0 else set()
        by_per_ap = self.demand.data_attr.get("by_prod_per_animal_prod")
        bp_animals = set(by_per_ap.index.unique("by_prod")) if len(by_per_ap) > 0 else set()

        all_byprods = bp_feeds | bp_feed_gen | bp_demand | bp_crops | bp_animals

        pss = self.x_idx["ani"].unique("prod_system")
        row_idx = pd.MultiIndex.from_product(
            [pss, sorted(all_byprods)],
            names=["prod_system", "by_prod"],
        )

        b10 = b10_fixed.reindex(row_idx, fill_value=0)

        # Supply side: animal numbers → by-product generation (national)
        A10_ani = self.make_A10_ani(row_idx)
        # Supply side: crop areas → by-product generation (national)
        A10_crp = self.make_A10_crp(row_idx)

        # Net feed contribution = feed generation (positive) + feed demand (negative)
        # + correction to remove by-products that make_A10_crp over-counts for crop areas used as feed
        A10_fds_demand = self.make_A1_3(row_idx=row_idx, prod_type="by_prod")
        A10_fds_gen = self.make_A1_3_gen(row_idx=row_idx)
        A10_fds_crop_corr = self.make_A10_fds_by_prod_corr(row_idx)
    ...
    def make_C15(self):
        ...
        # Feed demand side: feeds consuming by-products regionally
        fe_to_bp = self._get_feed_to_prod_factors('by_prod', drop_region=False)
        row_idx_demand = (
            fe_to_bp
            .loc[(fe_to_bp['share_domestic'] > 0) & (fe_to_bp['share_regional'] > 0)]
            .set_index(['prod_system', 'by_prod', 'region'])
            .index.unique()
        )

        # Feed generation side: feeds generating by-products regionally (feed_to_by_prod)
        fe_to_bp_gen = self._get_feed_to_prod_factors(
            'by_prod', drop_region=False, par_name='feed_to_by_prod'
        )
        if not fe_to_bp_gen.empty:
            row_idx_gen = (
                fe_to_bp_gen
                .loc[(fe_to_bp_gen['share_domestic'] > 0) & (fe_to_bp_gen['share_regional'] > 0)]
                .set_index(['prod_system', 'by_prod', 'region'])
                .index.unique()
            )
        else:
            row_idx_gen = pd.MultiIndex.from_tuples(
                [], names=['prod_system', 'by_prod', 'region']
            )

        row_idx = row_idx_demand.union(row_idx_gen)

        # Map animal numbers --> Generated by-products
        A15_ani = self.make_A10_ani(row_idx)
        # Map crop areas --> Generated by-products
        A15_crp = self.make_A10_crp(row_idx)
        # Map feeds --> Used by-products (negative)
        A15_fds = self.make_A15_fds(row_idx)
        # Map feeds --> Generated by-products via feed_to_by_prod (positive)
        A15_fds_gen = self.make_A15_fds_gen(row_idx)
        # Correction: remove by-products that make_A10_crp over-counts for crop areas used as feed
        A15_fds_crop_corr = self.make_A10_fds_by_prod_corr(row_idx)

        # Construct final matrix: supply (A15_ani, A15_crp, A15_fds_gen) - demand (A15_fds) >= 0
        A15 = IndexedMatrix(
            scipy.sparse.hstack([A15_ani.M, A15_crp.M, A15_fds.M + A15_fds_gen.M + A15_fds_crop_corr.M]),
            row_idx=row_idx,
            col_idx={
                "ani": self.x_idx["ani"],
                "crp": self.x_idx["crp"],
    ...
    def make_C16(self):
        """
        Ensure that generated crop residues (e.g. straw) meets demand for feed and
        bedding material as well as any food or non-food demand from DemandAndConversions.
        """
    ...
    def make_A1_3_gen(self, row_idx: pd.MultiIndex) -> "IndexedMatrix":
        """
        Matrix that converts feeds to *generated* by-products at a national level
        using the 'feed_to_by_prod' parameter (positive sign — production, not demand).

        Returns a zero matrix if no feed_to_by_prod factors are defined.
        Used in C10 to offset feed-driven by-product demand with feed-driven supply.
        """
        col_idx = self.x_idx["fds"]

        factors = self._get_feed_to_prod_factors("by_prod", par_name="feed_to_by_prod")
        if factors.empty:
            return IndexedMatrix.zeros(row_idx, col_idx)

        # Positive sign: generation adds to supply (domestic share applied)
        factors["feed_to_prod"] *= factors["share_domestic"]

        row_idx_df = row_idx.to_frame(index=False).reset_index(names="row_i")
        col_idx_df = col_idx.to_frame(index=False).reset_index(names="col_i")

        merged = row_idx_df.merge(factors, on=["prod_system", "by_prod"]).merge(
            col_idx_df,
            on=["feed", "animal", "species", "breed", "prod_system", "sub_system"],
        )
    ...
    def make_A2_2(self, row_idx: pd.MultiIndex, factors_with_reg_share: pd.DataFrame):
        ...
        merged = row_idx_df.merge(factors, on=["prod_system", "crop_prod", "region"]).merge(
    ...
    def make_A10_ani(self, row_idx: pd.MultiIndex) -> "IndexedMatrix":
        """
        Matrix mapping animal numbers to generated by-products.

        Handles both regional row_idx (prod_system, by_prod, region) as used in
        C15, and national row_idx (prod_system, by_prod) as used in C10.
        """
        col_idx = self.x_idx["ani"]
        ...
        regional = 'region' in row_idx.names
        val = []
        row_nr = []
        col_nr = []
                    ...
                    for bp in bps_f.index:
                        if (ops, bp) in row_idx:
                            ...
                            if (res != 0).any():
                                ...
                                if regional:
                                    row_nr.extend(
                                        [row_idx.get_loc((ops, bp, re)) for re in res.index]
                                    )
                                else:
                                    row_nr.extend([row_idx.get_loc((ops, bp))] * len(res))
                                col_nr.extend(
    ...
    def make_A10_crp(self, row_idx: pd.MultiIndex) -> "IndexedMatrix":
        """
        Matrix mapping crop areas to generated by-products.
        ...
        net_production = self._get_crop_production()
        by_per_cp_raw = self.demand.data_attr.get('by_prod_per_crop_prod')

        if len(by_per_cp_raw) == 0:
            return IndexedMatrix.zeros(row_idx, col_idx)

        by_per_cp = by_per_cp_raw.reset_index(name='by_prod_per_crop_prod')

        regional = 'region' in row_idx.names
        row_idx_ps_bp = row_idx.droplevel('region').unique() if regional else row_idx
        if len(
            set(by_per_cp.set_index(['prod_system', 'by_prod']).index) & set(row_idx_ps_bp)
        ) == 0:
            return IndexedMatrix.zeros(row_idx, col_idx)

        by_production = net_production.merge(by_per_cp, on=["prod_system", "crop_prod"])
        by_production["values"] = (
            by_production["net_production"] * by_production["by_prod_per_crop_prod"]
        )
        by_production = by_production.loc[by_production["values"] != 0]

        merge_on = ["prod_system", "by_prod"] + (["region"] if regional else [])
        merged = by_production.merge(
            row_idx_df, on=merge_on
        ).merge(
            col_idx_df, on=["crop", "prod_system", "region"]
        )

        return IndexedMatrix.from_frame(merged, row_idx, col_idx)

    def make_A15_fds(self,row_idx):
        """
    ...
    def make_A15_fds_gen(self, row_idx):
        """
        Create a matrix mapping feeds to *generated* by-products at regional level
        using the 'feed_to_by_prod' parameter.
        Positive sign: generation adds to regional by-product supply.
        Returns a zero matrix if no feed_to_by_prod factors are defined.
        """
        col_idx = self.x_idx["fds"]
        ...
        if fe_to_bp_gen.empty:
            return IndexedMatrix.zeros(row_idx, col_idx)

        fe_to_bp_gen = fe_to_bp_gen.loc[
            (fe_to_bp_gen['share_domestic'] > 0) & (fe_to_bp_gen['share_regional'] > 0)
        ]
        if fe_to_bp_gen.empty:
            return IndexedMatrix.zeros(row_idx, col_idx)
        ...
        merged = fe_to_bp_gen.merge(
            row_idx_df, on=["prod_system", "by_prod", "region"]
        ).merge(
            col_idx_df,
            on=["feed", "animal", "species", "breed", "prod_system", "sub_system", "region"],
        )
    ...
    def make_A10_fds_by_prod_corr(self, row_idx: pd.MultiIndex) -> "IndexedMatrix":
        """
        Correction to the fds block that removes double-counting of by-products from
        crop areas diverted to feed use.

        make_A10_crp credits by-product generation from total crop production
        (food + feed portions). But by-products from the feed-use portion are
        already handled by feed_to_by_prod (make_A1_3_gen / make_A15_fds_gen).
        This matrix subtracts the over-count: for each unit of feed f consuming
        crop product cp, we remove the by-products that make_A10_crp incorrectly
        attributed to those crop areas via by_prod_per_crop_prod.

        Handles both regional row_idx (prod_system, by_prod, region) for C15 and
        national row_idx (prod_system, by_prod) for C10.
        """
        col_idx = self.x_idx["fds"]
        ...
        if len(by_per_cp) == 0:
            return IndexedMatrix.zeros(row_idx, col_idx)
        by_per_cp_df = by_per_cp.reset_index(name='by_prod_per_crop_prod')

        regional = 'region' in row_idx.names

        factors = self._get_feed_to_prod_factors('crop_prod', drop_region=not regional)
        if factors.empty:
            return IndexedMatrix.zeros(row_idx, col_idx)

        if regional:
            factors = factors.loc[
                (factors['share_domestic'] > 0) & (factors['share_regional'] > 0)
            ]
            if factors.empty:
                return IndexedMatrix.zeros(row_idx, col_idx)

        merged = factors.merge(by_per_cp_df, on=['prod_system', 'crop_prod'])
        if merged.empty:
            return IndexedMatrix.zeros(row_idx, col_idx)

        if regional:
            merged['values'] = (
                merged['feed_to_prod'] * merged['share_domestic'] * merged['share_regional']
                * merged['by_prod_per_crop_prod'] * (-1)
            )
        else:
            merged['values'] = (
                merged['feed_to_prod'] * merged['share_domestic']
                * merged['by_prod_per_crop_prod'] * (-1)
            )
        ...
        row_merge_on = ['prod_system', 'by_prod'] + (['region'] if regional else [])
        col_merge_on = (
            ['feed', 'animal', 'species', 'breed', 'prod_system', 'sub_system']
            + (['region'] if regional else [])
        )

        merged = merged.merge(row_idx_df, on=row_merge_on).merge(col_idx_df, on=col_merge_on)
        if merged.empty:
            return IndexedMatrix.zeros(row_idx, col_idx)

        # Multiple crop_prod entries may contribute to the same (row_i, col_i)
        merged = merged.groupby(['row_i', 'col_i'], as_index=False)['values'].sum()
    ...
    def _get_feed_to_prod_factors(
        ...
        index: bool = False,
        drop_region: bool = True,
        par_name: str = "feed_to_prod",
    ) -> pd.DataFrame:
        """
        Get a DataFrame mapping feed products to (by-)products, with their respective
        conversion factor as well as respective import- and regional shares.

        par_name controls which conversion parameter to fetch (default 'feed_to_prod';
        pass 'feed_to_by_prod' to get feed-to-by-product generation factors).
        The returned DataFrame always names the conversion column 'feed_to_prod'.
        """
        feed_par = self.feed_mgmt.par
        feed_par.clear()

        if par_name == "feed_to_prod":
            feed_to_prod: pd.DataFrame = feed_par.get_unique(["feed", prod_type])
        else:
            feed_to_prod: pd.DataFrame = feed_par.get_unique(
                ["feed", prod_type], f'parameter == "{par_name}"'
            )
        if drop_region:
            row_idx = self.x_idx["fds"].droplevel("region").unique().to_frame(index=False)
        else:
            row_idx = self.x_idx["fds"].to_frame(index=False)
        df = row_idx.merge(feed_to_prod, on="feed")

        if df.empty:
            # Return empty DataFrame with the expected columns
            cols = (
                list(row_idx.columns) + [prod_type, "feed_to_prod", "share_domestic", "share_regional"]
                + ([] if drop_region else [])
            )
            return pd.DataFrame(columns=cols)

        filters = {cname: df[cname].to_list() for cname in df.columns}
        df["feed_to_prod"] = feed_par.get(par_name, **filters)

        if par_name == "feed_to_prod":
            for p in ["share_domestic", "share_regional"]:
                df[p] = feed_par.get(p, **filters) / 100
        else:
            # For feed_to_by_prod: share_domestic/share_regional are defined per source
            # crop_prod, not per generated by_prod. Reset filter state and look up via
            # the source crop_prod of each feed (from feed_to_prod).
            feed_par.clear()
            _crop_df = feed_par.get_unique(
                ["feed", "crop_prod"], 'parameter == "feed_to_prod"'
            )
            feed_crop_map = dict(zip(_crop_df["feed"], _crop_df["crop_prod"]))
            sd_filters = {c: df[c].to_list() for c in df.columns
                          if c not in [prod_type, "feed_to_prod"]}
            sd_filters["crop_prod"] = [feed_crop_map.get(f) for f in df["feed"]]
            for p in ["share_domestic", "share_regional"]:
                df[p] = feed_par.get(p, **sd_filters) / 100

        if index:
            df = df.set_index(
                ["feed", "animal", "species", "breed", "prod_system", "sub_system"]
                + [prod_type] + (["region"] if not drop_region else [])
            )
